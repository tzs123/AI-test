...
class DocumentProcessor:
    ...
    def extract_text_from_doc(file_path: str) -> str:
        ...
        try:
            ...
            if completed.stderr.strip():
                logger.warning(f"antiword 提取旧版Word失败: {completed.stderr.strip()}")
        except Exception as e:
            logger.warning(f"antiword 不可用或旧版Word提取失败: {e}")

        try:
            with open(file_path, 'rb') as file:
                content = file.read(5 * 1024 * 1024).decode('utf-8', errors='ignore')
            return "\n".join(
                item.strip()
                for item in re.findall(r"[\x20-\x7E\u4e00-\u9fff]{4,}", content)
                if item.strip()
            )
        except Exception as e:
            logger.error(f"旧版Word兜底读取失败: {e}")
            return f"旧版Word读取失败: {str(e)}"
    ...
    @staticmethod
    def extract_text_from_structured_file(file_path: str, document_type: str) -> str:
        """从接口文档和结构化文本中提取内容。"""
        raw_text = DocumentProcessor.extract_text_from_txt(file_path)
        if not raw_text or raw_text.startswith('文本文件读取失败'):
            return raw_text

        try:
            if document_type in {'json', 'har'}:
                data = json.loads(raw_text)
                if document_type == 'har' and isinstance(data, dict):
                    ...
                    for index, entry in enumerate(entries, 1):
                        ...
                        header_names = ", ".join(
                            item.get('name')
                            for item in headers
                            if item.get('name') in {'Authorization', 'Content-Type', 'Accept', 'Cookie', 'X-Token'}
                        )
                        body_text = post_data.get('text') or ""
                        response_text = response_content.get('text') or ""
                        lines.append(
                            "\n".join([
                                f"{index}. {request.get('method', '')} {request.get('url', '')} -> {response.get('status', '')} {response.get('statusText', '')}",
                                f"   Query: {query_text or '无'}",
                                f"   Important Headers: {header_names or '无'}",
                                f"   Request Body: {body_text[:1000] or '无'}",
                                f"   Response Sample: {response_text[:1000] or response_content.get('mimeType', '') or '无'}",
            ...
            if document_type == 'yaml':
                ...
                return yaml.safe_dump(data, allow_unicode=True, sort_keys=False) if data is not None else raw_text

            if document_type == 'xml':
                root = ET.fromstring(raw_text)
                text_lines = [item.strip() for item in root.itertext() if item.strip()]
                return "\n".join(text_lines) or raw_text
        except Exception:
            return raw_text
    ...
    def extract_text_from_image(file_path: str) -> str:
        """使用本地 Tesseract OCR 提取需求截图中的中英文文字。"""
        try:
            ...
            with Image.open(file_path) as image:
                ...
                try:
                    text = pytesseract.image_to_string(grayscale, lang='chi_sim+eng')
                except pytesseract.TesseractError:
                    text = pytesseract.image_to_string(grayscale, lang='eng')
            text = text.strip()
            return text or '图片中未识别到文字，请上传更清晰的图片。'
        except Exception as e:
            logger.error(f"图片 OCR 提取失败: {e}")
            return f"图片 OCR 提取失败: {str(e)}"

    @staticmethod
    def _flatten_xmind_topic(topic: dict, level: int = 0) -> List[str]:
        lines = []
        ...
        for group_name in ('attached', 'detached', 'summary'):
            for child in children.get(group_name) or []:
                lines.extend(DocumentProcessor._flatten_xmind_topic(child, level + 1))
        return lines

    @staticmethod
    def extract_text_from_xmind(file_path: str) -> str:
        """解析新版 content.json 与旧版 content.xml 格式的 XMind 文件。"""
        try:
            with zipfile.ZipFile(file_path, 'r') as archive:
                ...
                if 'content.json' in names:
                    ...
                    for index, sheet in enumerate(sheets, 1):
                        sheet_title = str(sheet.get('title') or f'画布 {index}').strip()
                        lines.append(f"# {sheet_title}")
                        root_topic = sheet.get('rootTopic') or {}
                        lines.extend(DocumentProcessor._flatten_xmind_topic(root_topic))
                elif 'content.xml' in names:
                    root = ET.fromstring(archive.read('content.xml'))
                    for sheet_index, sheet in enumerate(root.findall('.//{*}sheet'), 1):
                        sheet_title = sheet.findtext('./{*}title') or f'画布 {sheet_index}'
                        lines.append(f"# {sheet_title}")

                        def walk_xml(topic, level=0):
                            title = topic.findtext('./{*}title') or ''
                            if title.strip():
                    ...
                    return '无法识别 XMind 文件结构：缺少 content.json 或 content.xml。'
            return '\n'.join(lines).strip() or 'XMind 文件中没有可读取的主题。'
        except (zipfile.BadZipFile, ET.ParseError, UnicodeDecodeError, json.JSONDecodeError) as e:
            logger.error(f"XMind 解析失败: {e}")
            return f"XMind 解析失败: {str(e)}"
        except Exception as e:
            logger.error(f"XMind 读取失败: {e}")
            return f"XMind 读取失败: {str(e)}"
    ...
    def extract_text(cls, document: RequirementDocument) -> str:
        ...
        elif document.document_type in {'yaml', 'json', 'har', 'xml', 'csv', 'sql'}:
            return cls.extract_text_from_structured_file(file_path, document.document_type)
        else:
            return "不支持的文档类型"
