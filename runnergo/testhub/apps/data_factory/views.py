...
class TestDataGenerateAPIView(APIView):
    """供 AI Agent 和 API 测试模块调用的标准测试数据生成接口。"""

    permission_classes = [IsAuthenticated]

    def post(self, request):
        data_type = str(request.data.get('type') or 'USER').upper()
        try:
            count = min(max(int(request.data.get('count') or 1), 1), 10000)
        except (TypeError, ValueError):
            return Response({'error': 'count 必须是数字'}, status=status.HTTP_400_BAD_REQUEST)
        fields = request.data.get('fields') or ['name', 'phone', 'email', 'password']
        if not isinstance(fields, list):
            return Response({'error': 'fields 必须是数组'}, status=status.HTTP_400_BAD_REQUEST)

        rows = []
        for index in range(count):
            row = {}
            if 'name' in fields:
                row['name'] = TestDataTools.generate_chinese_name().get('result')
            if 'phone' in fields:
                row['phone'] = TestDataTools.generate_chinese_phone().get('result')
            if 'email' in fields:
                row['email'] = TestDataTools.generate_chinese_email().get('result')
            if 'password' in fields:
                row['password'] = RandomTools.random_password(length=12).get('result')
            if 'address' in fields:
                row['address'] = TestDataTools.generate_chinese_address().get('result')
            if 'id_card' in fields:
                row['id_card'] = TestDataTools.generate_id_card().get('result')
            rows.append(row)

        asset_id = f'data_{uuid.uuid4().hex[:12]}'
        record = DataFactoryRecord.objects.create(
            user=request.user,
            tool_name='agent_generate_data',
            tool_category='test_data',
            tool_scenario='test_data',
            input_data={'type': data_type, 'count': count, 'fields': fields},
            output_data={'asset_id': asset_id, 'count': count, 'rows': rows},
            is_saved=True,
            tags=['AI Agent', data_type],
        )
        return Response({
            'asset_id': asset_id,
            'record_id': record.id,
...
class DataFactoryViewSet(viewsets.ModelViewSet):
    ...
    def execute_mock_tool(self, tool_name: str, input_data: Union[dict, str]):
        """执行Mock数据工具"""
        tool_mapping = {
