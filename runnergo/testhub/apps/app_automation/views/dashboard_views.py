...
class AppDashboardViewSet(viewsets.ViewSet):
    ...
    def statistics(self, request):
        ...
        try:
            ...
            # 测试用例统计
            cases = accessible_project_assets(
            ...
            if not is_platform_admin(request.user):
                executions = executions.filter(
                    Q(user=request.user)
                    | Q(test_case__project__owner=request.user)
                    | Q(test_case__project__members=request.user)
                    | Q(test_suite__project__owner=request.user)
                    | Q(test_suite__project__members=request.user)
                ).distinct()
            ...
            # 最近执行记录
            recent_executions = executions.order_by('-created_at')[:3]
