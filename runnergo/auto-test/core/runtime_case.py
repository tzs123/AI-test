"""Execution-time input overrides for Web UI flow cases.
...
def resolve_dynamic_value(value: Any) -> Any:
    ...
    Unknown expressions are intentionally preserved so existing environment or
    flow-variable resolvers can handle them later.
    """
    if isinstance(value, str):
        def replace(match: re.Match) -> str:
            ...
            if name in {"random_code", "random_digits"}:
                length = _parse_length(args_text, 6)
                return "".join(random.choice(string.digits) for _ in range(length))
            if name == "random_idcard":
                provinces = ["110101", "310101", "440101", "440301", "330101",
                             "420101", "510101", "320101", "500101"]
                prov = random.choice(provinces)
                year = 1960 + random.randint(0, 40)
                month = f"{1 + random.randint(0, 11):02d}"
                day = f"{1 + random.randint(0, 27):02d}"
                seq = "".join(random.choice(string.digits) for _ in range(3))
                base = f"{prov}{year}{month}{day}{seq}"
                weights = [7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2]
                check_map = "10X98765432"
                s = sum(int(base[i]) * weights[i] for i in range(17))
                return base + check_map[s % 11]
            if name == "random_email":
                names = ["user", "test", "admin", "dev", "qa", "ops"]
                domains = ["gmail.com", "qq.com", "163.com", "126.com", "outlook.com"]
                return random.choice(names) + uuid.uuid4().hex[:8] + "@" + random.choice(domains)
            if name == "random_name":
                surnames = "张王李赵刘陈杨黄周吴徐孙马朱胡林何高罗郑"
                given = "伟芳娜敏静丽强磊洋勇军杰涛明超秀霞平刚桂"
                return random.choice(surnames) + random.choice(given) + random.choice(given)
            if name == "random_plate":
                provinces = "京沪粤浙苏鲁川鄂湘闽陕豫冀皖赣桂晋蒙辽吉黑云贵渝津"
                letters = "ABCDEFGHJKLMNPQRSTUVWXYZ"
                plate = random.choice(provinces) + random.choice(letters)
                for _ in range(5):
                    plate += random.choice(string.digits + letters) if random.random() < 0.5 else random.choice(string.digits)
                return plate
            if name == "random_amount":
                return f"{random.random() * 10000:.2f}"
...
def _flow_steps_container(case_data: Any) -> Tuple[Optional[List[Any]], Tuple[Any, ...]]:
    if isinstance(case_data, list):
        return case_data, ()
    if isinstance(case_data, dict) and isinstance(case_data.get("steps"), list):
        return case_data["steps"], ("steps",)
    return None, ()
...
def _find_step_insert_location(
    ...
    target_action = str(rule.get("afterAction") or rule.get("after_action") or "").strip().lower()
...
def _runtime_assertion_step(rule: Dict[str, Any], index: int) -> Dict[str, Any]:
    action = _normalize_assertion_type(rule.get("type") or rule.get("action"))
    if action not in {"assert_text", "assert_url", "assert_visible", "assert_error"}:
        raise ValueError(f"runtime_assertions 第 {index} 项断言类型不支持: {action}")

    expected = rule.get("expected", rule.get("value", rule.get("runtimeValue", "")))
    step: Dict[str, Any] = {
        "id": str(rule.get("id") or f"runtime-assertion-{index}"),
        "action": action,
        "name": str(rule.get("name") or "数据驱动断言"),
        "timeout": rule.get("timeout", 10000),
        "runtime_generated_assertion": True,
        "runtime_assertion_rule": copy.deepcopy(rule),
    }
    if action in {"assert_text", "assert_url", "assert_error"}:
        step["value"] = expected
    if rule.get("keywords"):
        step["keywords"] = copy.deepcopy(rule.get("keywords"))
    if rule.get("message"):
        step["message"] = rule.get("message")
    if rule.get("element"):
        step["element"] = copy.deepcopy(rule.get("element"))
    return step


def inject_runtime_assertions(case_data: Any) -> Any:
    ...
    steps, _ = _flow_steps_container(root)
    if steps is None:
    ...
    for index, rule in enumerate(rules, 1):
        ...
        if not location:
            label = rule.get("afterStepId") or rule.get("afterStepPath") or rule.get("afterAction") or "末尾"
            raise ValueError(f"runtime_assertions 第 {index} 项找不到插入步骤: {label}")
        parent, insert_index = location
        parent.insert(insert_index, _runtime_assertion_step(rule, index))
    return root
...
class RuntimeCase:
    ...
    def build(self) -> "RuntimeCase":
        ...
        for field in self.fields:
            ...
            final_value = resolve_dynamic_value(raw_value)
        ...
        self.case_data = inject_runtime_assertions(self.case_data)
        return self
